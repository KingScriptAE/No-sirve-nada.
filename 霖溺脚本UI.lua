...
function Ripple(obj)
	spawn(function()
		...
		Ripple.ZIndex = 8
		Ripple.Image = "rbxassetid://125385332140299"--这里和背景必须一样
...
if theme == 'dark' then
    ...
    beijingColor = Color3.fromRGB(25, 25, 25)
      ...
      DropShadow.Image = "rbxassetid://125385332140299" --背景
...
Open.Active = true
Open.Draggable = true
Open.Image = "rbxassetid://125385332140299"--UI圆形图片
Open.MouseButton1Click:Connect(function()
  Main.Visible = not Main.Visible
  Open.Image = Main.Visible and "rbxassetid://125385332140299" or "rbxassetid://125385332140299" --开和关的图
end)
...
border.Color = ALcolor:Lerp(Color3.new(1,1,1), 0.3)  
border.Thickness = 2
...
services.TweenService:Create(ToggleSwitch, TweenInfo.new(0.2, Enum.EasingStyle.Bounce), {
    ...
    BackgroundColor3 = (state and zyColor or beijingColor)  
}):Play()
...
DropdownModule.ClipsDescendants = false  
  ...
  services.TweenService:Create(DropdownTop, TweenInfo.new(0.2), {
    BackgroundTransparency = ALTransparency
            ...
            local searchDropdown = function(text)
                if not open then return end  -- [!] 新增此行
              local options = DropdownModule:GetChildren()
              for i=1, #options do
                local option = options[i]
                if text == "" then
...
local ToggleDropVis = function()
    open = not open
    if open then 
        setAllVisible()  -- 展开时显示所有选项
        DropdownModule.Visible = true
    else
        -- ▼ 关闭时隐藏所有选项 ▼
        for _, child in ipairs(DropdownModule:GetChildren()) do
  ...
  services.TweenService:Create(DropdownModule, TweenInfo.new(
    0.4, 
    Enum.EasingStyle.Bounce,  -- 弹性动画
    Enum.EasingDirection.Out
), {
    Size = UDim2.new(0, 428, 0, math.clamp(targetSize, 38, 300))  -- 限制最大高度
}):Play()
  -- 箭头旋转动画
