...
function Ripple(obj)
	spawn(function()
		...
		Ripple.ZIndex = 8
		Ripple.Image = "rbxassetid://105743192733444"--这里和背景必须一样
...
----------------HACKER---------------------
ALTransparency = 0.6
ALcolor = Color3.fromRGB(255, 0, 0)  --脚本字体颜色
--------------HACKER------------------------
if theme == 'dark' then
    ...
    beijingColor = Color3.fromRGB(25, 25, 25)
      ...
      DropShadow.Image = "rbxassetid://105743192733444" --背景
...
Open.Active = true
Open.Draggable = true
Open.Image = "rbxassetid://105743192733444"--UI圆形图片
Open.MouseButton1Click:Connect(function()
  Main.Visible = not Main.Visible
...
TextLabel.TextTransparency = 1  -- 强制不透明（防覆盖）
TextLabel.BackgroundTransparency = 1  -- 调高背景透明度（减少遮盖）

-- 新增：黑色描边提升对比度（关键修改）
local textStroke = Instance.new("UIStroke", TextLabel)
textStroke.Color = Color3.new(0, 0, 0)  -- 黑色描边
textStroke.Thickness = 0.7
textStroke.Transparency = 0.3

-- 修改渐变背景为更暗（减少干扰）
local gradient = Instance.new("UIGradient", TextLabel)
gradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, zyColor:Lerp(Color3.new(0,0,0), 0.4)),  -- 暗色渐变
    ColorSequenceKeypoint.new(1, zyColor:Lerp(Color3.new(0,0,0), 0.6))
}
gradient.Rotation = 90


-- 立体边框（动态颜色）
local border = Instance.new("UIStroke", TextLabel)
border.Color = ALcolor:Lerp(Color3.new(1,1,1), 0.3)  -- 浅色边框
border.Thickness = 2
border.Transparency = 0.2

-- 悬浮高光（交互反馈）
TextLabel.MouseEnter:Connect(function()
    services.TweenService:Create(gradient, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
        Rotation = 45,
        Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0, zyColor:Lerp(Color3.new(1,1,1), 0.1)),
            ColorSequenceKeypoint.new(1, zyColor)
        }
    }):Play()
    services.TweenService:Create(border, TweenInfo.new(0.2), {
        Color = ALcolor:Lerp(Color3.new(1,1,1), 0.5)
    }):Play()
end)

TextLabel.MouseLeave:Connect(function()
    services.TweenService:Create(gradient, TweenInfo.new(0.2), {Rotation = 90}):Play()
    services.TweenService:Create(border, TweenInfo.new(0.2), {Color = ALcolor}):Play()
end)
...
-- 内阴影（模拟凹陷感）
local innerShadow = Instance.new("UIStroke", ToggleSwitch)
...
-- 发光边框（主题色动态匹配）
local glow = Instance.new("UIStroke", ToggleSwitch)
glow.Color = ALcolor  -- 使用全局文字色
glow.Thickness = 1.5
glow.Transparency = 0.3
glow.LineJoinMode = Enum.LineJoinMode.Round

-- 弹性动画（替换原切换动画）
services.TweenService:Create(ToggleSwitch, TweenInfo.new(0.2, Enum.EasingStyle.Bounce), {
    Position = UDim2.new(0, (state and 18 or 0), 0, 0),  -- 动态计算位置
    BackgroundColor3 = (state and zyColor or beijingColor)  -- 使用主题背景色
}):Play()
...
DropdownModule.ClipsDescendants = false  -- [!] 关闭裁剪
DropdownModule.Position = UDim2.new(0, 0, 0, 0)
DropdownModule.Size = UDim2.new(0, 428, 0, 38)  -- 初始高度为收起状态
DropdownModule.Visible = true  -- [!] 强制可见

     --[[       -- 添加初始缩放动画
services.TweenService:Create(DropdownModule, TweenInfo.new(0.3, Enum.EasingStyle.Back), {
    Size = UDim2.new(0, 428, 0, 0)
}):Play() -- 初始隐藏
]]
            ...
            DropdownTop.TextXAlignment = Enum.TextXAlignment.Left
                        -- 添加立体阴影
            local dropdownShadow = Instance.new("UIStroke", DropdownTop)
dropdownShadow.Color = Color3.new(0,0,0)
dropdownShadow.Thickness = 2
dropdownShadow.Transparency = 0.3
-- 添加动态发光边框
local dropdownGlow = Instance.new("UIStroke", DropdownTop)
dropdownGlow.Color = ALcolor
dropdownGlow.Thickness = 1.5
dropdownGlow.Transparency = 0.6
dropdownGlow.LineJoinMode = Enum.LineJoinMode.Round
            -- 添加拖动逻辑
local dragInput, dragStart, startPos
DropdownTop.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragStart = input.Position
        startPos = DropdownModule.Position
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                dragInput = nil
            end
...
DropdownTop.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement then
        dragInput = input
    end
end)
...
services.UserInputService.InputChanged:Connect(function(input)
    if input == dragInput then
        local delta = input.Position - dragStart
        DropdownModule.Position = UDim2.new(
            startPos.X.Scale, 
...
local ToggleDropVis = function()
  ...
  if open then 
    setAllVisible()
    DropdownModule.Visible = true  -- [!] 确保展开时可见
  end
  DropdownOpen.Text = (open and "-" or "+")
  
  -- 弹性动画 + 修复高度计算
  local targetSize = open and (DropdownModuleL.AbsoluteContentSize.Y + 4) or 38
  services.TweenService:Create(DropdownModule, TweenInfo.new(0.4, Enum.EasingStyle.Bounce), {
    Size = UDim2.new(0, 428, 0, math.clamp(targetSize, 38, 500))  -- [!] 限制最大高度
  }):Play()
  -- 箭头旋转动画
  services.TweenService:Create(DropdownOpen, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
...
Option.MouseEnter:Connect(function()
  -- [!] 添加条件判断，避免在动画过程中重复触发
  if not open then return end
  services.TweenService:Create(Option, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
    Size = UDim2.new(0, 435, 0, 30),
    BackgroundColor3 = ALcolor:Lerp(Color3.new(1,1,1), 0.1)
  }):Play()
end)

Option.MouseLeave:Connect(function()
  services.TweenService:Create(Option, TweenInfo.new(0.3, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {
...
DropdownModuleL:GetPropertyChangedSignal("AbsoluteContentSize"):Wait()
  DropdownModuleL:ApplyLayout()
