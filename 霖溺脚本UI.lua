...
local regionCode = "未知地区"
task.spawn(function()
    local success, result = pcall(function()
        return LocalizationService:GetCountryRegionForPlayerAsync(player)
    end)
    if success then
        regionCode = result or "未知地区"
    end
end)
...
ScreenGui.Name = "LinniLoaderUI_" .. HttpService:GenerateGUID(false):sub(1,8)
...
local function explodeParticles(particles, centerPosition)
    for i, particle in ipairs(particles) do
        local angle = math.rad((i / #particles) * 360)
        local distance = math.random(50, 150)
        local targetX = centerPosition.X.Scale + math.cos(angle) * distance/450
        local targetY = centerPosition.Y.Scale + math.sin(angle) * distance/220
...
PERCENT.Parent = ProgressContainer
PERCENT.BackgroundTransparency = 1
PERCENT.Position = UDim2.new(1, -45, 0, 0)
PERCENT.Size = UDim2.new(0, 40, 1, 0)
PERCENT.Font = Enum.Font.GothamBold
PERCENT.Text = "0%"
PERCENT.TextColor3 = Color3.fromRGB(0, 255, 255)
PERCENT.TextSize = 14
...
LOADING.Position = UDim2.new(0.35, 0, 0.4, 0)
LOADING.Size = UDim2.new(0.6, 0, 0, 40)
...
VersionLabel.Position = UDim2.new(0.8, 0, 0.9, 0)
VersionLabel.Size = UDim2.new(0.18, 0, 0, 15)
...
LOGO.Image = "rbxassetid://17120415981"
LOGO.ImageColor3 = Color3.fromRGB(255, 255, 255)
LOGO.Visible = false

local function createClones(original, count)
    local clones = {}
    for i = 1, count do
...
local function FastTypewriterEffect(label, text, speed)
    label.Text = ""
    for i = 1, #text do
...
local function LinniLoader()
    ...
    local particles = createParticles(MAIN, 30, Color3.fromRGB(0, 200, 255))
    ...
    TweenService:Create(UICorner, TweenInfo.new(0.5, Enum.EasingStyle.Quad), {
        CornerRadius = UDim.new(0, 12)
    }):Play()
    
    explodeParticles(particles, MAIN.Position)
    ...
    local logoClones = createClones(LOGO, 3)
    for i, clone in ipairs(logoClones) do
        clone.ImageTransparency = 0.7
        clone.ZIndex = -1
        
        local offsetX = (i-2) * 10
        local offsetY = (i-2) * 5
        
        TweenService:Create(clone, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
            Position = UDim2.new(0.05, offsetX, 0.2, offsetY),
            Size = UDim2.new(0, 90, 0, 90),
            ImageColor3 = Color3.fromHSV(i/6, 0.8, 1)
        }):Play()
    end
    
    local loadingSteps = {
        {text = "初始化Logo加载...", size = 0.2, duration = 0.5},
        {text = "连接github服务器...", size = 0.4, duration = 0.5},
        {text = "作者: 霖溺 | 版本: MAX", size = 0.6, duration = 0.5},
        {text = "欢迎尊贵用户 "..player.DisplayName.." ("..regionCode..")", size = 0.8, duration = 0.5},
        {text = "祝你玩的愉快!", size = 1.0, duration = 0.5}
    }
    
    for i, step in ipairs(loadingSteps) do
        task.spawn(FastTypewriterEffect, LOADING, step.text, 0.02)
        
        local tween = TweenService:Create(BAR, TweenInfo.new(step.duration, Enum.EasingStyle.Quad), {
            Size = UDim2.new(step.size, 0, 1, 0)
        })
        tween:Play()
        
        local hue = (step.size) * 120
        BAR.BackgroundColor3 = Color3.fromHSV(hue/360, 0.8, 1)
        barGlow.Color = Color3.fromHSV((hue+30)/360, 0.9, 1)
        
        for p = tonumber(PERCENT.Text:sub(1, -2)) or 0, step.size * 100, 5 do
            PERCENT.Text = string.format("%.0f%%", p)
            task.wait(0.01)
        end
        PERCENT.Text = string.format("%.0f%%", step.size * 100)
        
        task.wait(step.duration)
    end
    
    local finalParticles = createParticles(MAIN, 50, Color3.fromRGB(0, 255, 255))
    explodeParticles(finalParticles, MAIN.Position)
    
    TweenService:Create(MAIN, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
