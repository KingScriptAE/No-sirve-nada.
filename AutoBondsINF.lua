...
icon.Image = "rbxassetid://99599917888886"
...
notify("自动债券", "这只是测试版，所以可能会有一些错误/bug.", 10)


local Notif = loadstring(game:HttpGet("https://raw.githubusercontent.com/KingScriptAE/No-sirve-nada./refs/heads/main/Notif.lua"))()
if game.PlaceId == 70876832253163 or game.PlaceId == 98018823628597 then
    Notif.New("[Linni Hub] 自动债券已加载!", 5)

    toggleMinimize()

    local success, err = pcall(function()--备用
        loadstring(game:HttpGet("https://raw.githubusercontent.com/KingScriptAE/No-sirve-nada./refs/heads/main/%E5%A4%87%E7%94%A8.lua"))()
    end)

    if not success then
        warn("[Linni Hub] 自动债券脚本加载失败:", err)
    end
end